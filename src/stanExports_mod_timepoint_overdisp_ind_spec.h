// Generated by rstantools.  Do not edit by hand.

/*
    epipool is free software: you can redistribute it and/or modify
    it under the terms of the GNU General Public License as published by
    the Free Software Foundation, either version 3 of the License, or
    (at your option) any later version.

    epipool is distributed in the hope that it will be useful,
    but WITHOUT ANY WARRANTY; without even the implied warranty of
    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
    GNU General Public License for more details.

    You should have received a copy of the GNU General Public License
    along with epipool.  If not, see <http://www.gnu.org/licenses/>.
*/
#ifndef MODELS_HPP
#define MODELS_HPP
#define STAN__SERVICES__COMMAND_HPP
#ifndef USE_STANC3
#define USE_STANC3
#endif
#include <rstan/rstaninc.hpp>
// Code generated by %%NAME%% %%VERSION%%
#include <stan/model/model_header.hpp>
namespace model_mod_timepoint_overdisp_ind_spec_namespace {
inline void validate_positive_index(const char* var_name, const char* expr,
                                    int val) {
  if (val < 1) {
    std::stringstream msg;
    msg << "Found dimension size less than one in simplex declaration"
        << "; variable=" << var_name << "; dimension size expression=" << expr
        << "; expression value=" << val;
    std::string msg_str(msg.str());
    throw std::invalid_argument(msg_str.c_str());
  }
}
inline void validate_unit_vector_index(const char* var_name, const char* expr,
                                       int val) {
  if (val <= 1) {
    std::stringstream msg;
    if (val == 1) {
      msg << "Found dimension size one in unit vector declaration."
          << " One-dimensional unit vector is discrete"
          << " but the target distribution must be continuous."
          << " variable=" << var_name << "; dimension size expression=" << expr;
    } else {
      msg << "Found dimension size less than one in unit vector declaration"
          << "; variable=" << var_name << "; dimension size expression=" << expr
          << "; expression value=" << val;
    }
    std::string msg_str(msg.str());
    throw std::invalid_argument(msg_str.c_str());
  }
}
using std::istream;
using std::string;
using std::stringstream;
using std::vector;
using std::pow;
using stan::io::dump;
using stan::math::lgamma;
using stan::model::model_base_crtp;
using stan::model::rvalue;
using stan::model::cons_list;
using stan::model::index_uni;
using stan::model::index_max;
using stan::model::index_min;
using stan::model::index_min_max;
using stan::model::index_multi;
using stan::model::index_omni;
using stan::model::nil_index_list;
using namespace stan::math;
using stan::math::pow; 
stan::math::profile_map profiles__;
static int current_statement__= 0;
static const std::vector<string> locations_array__ = {" (found before start of program)",
                                                      " (in 'string', line 30, column 2 to column 40)",
                                                      " (in 'string', line 31, column 2 to column 30)",
                                                      " (in 'string', line 32, column 2 to column 29)",
                                                      " (in 'string', line 35, column 2 to column 39)",
                                                      " (in 'string', line 36, column 2 to column 34)",
                                                      " (in 'string', line 37, column 2 to column 25)",
                                                      " (in 'string', line 39, column 4 to column 43)",
                                                      " (in 'string', line 38, column 19 to line 40, column 3)",
                                                      " (in 'string', line 38, column 2 to line 40, column 3)",
                                                      " (in 'string', line 42, column 4 to column 120)",
                                                      " (in 'string', line 41, column 15 to line 43, column 3)",
                                                      " (in 'string', line 41, column 2 to line 43, column 3)",
                                                      " (in 'string', line 67, column 2 to column 38)",
                                                      " (in 'string', line 70, column 6 to column 49)",
                                                      " (in 'string', line 69, column 23 to line 71, column 5)",
                                                      " (in 'string', line 69, column 4 to line 71, column 5)",
                                                      " (in 'string', line 68, column 19 to line 72, column 3)",
                                                      " (in 'string', line 68, column 2 to line 72, column 3)",
                                                      " (in 'string', line 48, column 4 to column 61)",
                                                      " (in 'string', line 47, column 19 to line 49, column 3)",
                                                      " (in 'string', line 47, column 2 to line 49, column 3)",
                                                      " (in 'string', line 50, column 2 to column 27)",
                                                      " (in 'string', line 51, column 2 to column 36)",
                                                      " (in 'string', line 58, column 8 to column 58)",
                                                      " (in 'string', line 57, column 11 to line 59, column 7)",
                                                      " (in 'string', line 55, column 8 to line 56, column 94)",
                                                      " (in 'string', line 54, column 27 to line 57, column 7)",
                                                      " (in 'string', line 54, column 6 to line 59, column 7)",
                                                      " (in 'string', line 53, column 17 to line 60, column 5)",
                                                      " (in 'string', line 53, column 4 to line 60, column 5)",
                                                      " (in 'string', line 62, column 6 to column 59)",
                                                      " (in 'string', line 61, column 22 to line 63, column 5)",
                                                      " (in 'string', line 61, column 4 to line 63, column 5)",
                                                      " (in 'string', line 52, column 18 to line 64, column 3)",
                                                      " (in 'string', line 52, column 2 to line 64, column 3)",
                                                      " (in 'string', line 3, column 2 to column 8)",
                                                      " (in 'string', line 4, column 2 to column 10)",
                                                      " (in 'string', line 5, column 2 to column 12)",
                                                      " (in 'string', line 6, column 8 to column 9)",
                                                      " (in 'string', line 6, column 2 to column 22)",
                                                      " (in 'string', line 7, column 8 to column 9)",
                                                      " (in 'string', line 7, column 2 to column 19)",
                                                      " (in 'string', line 8, column 8 to column 11)",
                                                      " (in 'string', line 8, column 2 to column 20)",
                                                      " (in 'string', line 9, column 8 to column 9)",
                                                      " (in 'string', line 9, column 2 to column 18)",
                                                      " (in 'string', line 10, column 8 to column 9)",
                                                      " (in 'string', line 10, column 2 to column 17)",
                                                      " (in 'string', line 11, column 8 to column 9)",
                                                      " (in 'string', line 11, column 2 to column 17)",
                                                      " (in 'string', line 13, column 2 to column 24)",
                                                      " (in 'string', line 14, column 8 to column 14)",
                                                      " (in 'string', line 14, column 2 to column 39)",
                                                      " (in 'string', line 15, column 8 to column 14)",
                                                      " (in 'string', line 15, column 2 to column 39)",
                                                      " (in 'string', line 17, column 2 to column 23)",
                                                      " (in 'string', line 18, column 2 to column 28)",
                                                      " (in 'string', line 19, column 2 to column 12)",
                                                      " (in 'string', line 21, column 2 to column 12)",
                                                      " (in 'string', line 22, column 8 to column 13)",
                                                      " (in 'string', line 22, column 2 to column 27)",
                                                      " (in 'string', line 23, column 2 to column 13)",
                                                      " (in 'string', line 24, column 2 to column 16)",
                                                      " (in 'string', line 27, column 2 to column 20)",
                                                      " (in 'string', line 30, column 8 to column 13)",
                                                      " (in 'string', line 30, column 22 to column 25)",
                                                      " (in 'string', line 32, column 18 to column 23)",
                                                      " (in 'string', line 35, column 18 to column 23)",
                                                      " (in 'string', line 36, column 8 to column 13)",
                                                      " (in 'string', line 36, column 22 to column 25)",
                                                      " (in 'string', line 37, column 8 to column 9)",
                                                      " (in 'string', line 67, column 8 to column 13)",
                                                      " (in 'string', line 67, column 15 to column 20)"};
#include <stan_meta_header.hpp>
class model_mod_timepoint_overdisp_ind_spec final : public model_base_crtp<model_mod_timepoint_overdisp_ind_spec> {
private:
  int N;
  int N_t;
  int N_pop;
  std::vector<int> rank_t;
  std::vector<int> pop;
  std::vector<double> t;
  std::vector<double> s;
  std::vector<int> n;
  std::vector<int> k;
  int J_sens;
  std::vector<int> y_sens;
  std::vector<int> n_sens;
  std::vector<double> p_sens;
  std::vector<double> p_intercept;
  double spec;
  int N_phi;
  std::vector<int> use_phi;
  double p_phi;
  int inference;
  double delta;
  int prev_ratio_2dim__;
 
public:
  ~model_mod_timepoint_overdisp_ind_spec() { }
  
  inline std::string model_name() const final { return "model_mod_timepoint_overdisp_ind_spec"; }
  inline std::vector<std::string> model_compile_info() const noexcept {
    return std::vector<std::string>{"stanc_version = %%NAME%%3 %%VERSION%%", "stancflags = "};
  }
  
  
  model_mod_timepoint_overdisp_ind_spec(stan::io::var_context& context__,
                                        unsigned int random_seed__ = 0,
                                        std::ostream* pstream__ = nullptr) : model_base_crtp(0) {
    using local_scalar_t__ = double ;
    boost::ecuyer1988 base_rng__ = 
        stan::services::util::create_rng(random_seed__, 0);
    (void) base_rng__;  // suppress unused var warning
    static const char* function__ = "model_mod_timepoint_overdisp_ind_spec_namespace::model_mod_timepoint_overdisp_ind_spec";
    (void) function__;  // suppress unused var warning
    local_scalar_t__ DUMMY_VAR__(std::numeric_limits<double>::quiet_NaN());
    (void) DUMMY_VAR__;  // suppress unused var warning
    
    try {
      int pos__;
      pos__ = std::numeric_limits<int>::min();
      
      pos__ = 1;
      current_statement__ = 36;
      context__.validate_dims("data initialization","N","int",
          context__.to_vec());
      N = std::numeric_limits<int>::min();
      
      current_statement__ = 36;
      N = context__.vals_i("N")[(1 - 1)];
      current_statement__ = 37;
      context__.validate_dims("data initialization","N_t","int",
          context__.to_vec());
      N_t = std::numeric_limits<int>::min();
      
      current_statement__ = 37;
      N_t = context__.vals_i("N_t")[(1 - 1)];
      current_statement__ = 38;
      context__.validate_dims("data initialization","N_pop","int",
          context__.to_vec());
      N_pop = std::numeric_limits<int>::min();
      
      current_statement__ = 38;
      N_pop = context__.vals_i("N_pop")[(1 - 1)];
      current_statement__ = 39;
      validate_non_negative_index("rank_t", "N", N);
      current_statement__ = 40;
      context__.validate_dims("data initialization","rank_t","int",
          context__.to_vec(N));
      rank_t = std::vector<int>(N, std::numeric_limits<int>::min());
      
      current_statement__ = 40;
      assign(rank_t, nil_index_list(), context__.vals_i("rank_t"),
        "assigning variable rank_t");
      current_statement__ = 41;
      validate_non_negative_index("pop", "N", N);
      current_statement__ = 42;
      context__.validate_dims("data initialization","pop","int",
          context__.to_vec(N));
      pop = std::vector<int>(N, std::numeric_limits<int>::min());
      
      current_statement__ = 42;
      assign(pop, nil_index_list(), context__.vals_i("pop"),
        "assigning variable pop");
      current_statement__ = 43;
      validate_non_negative_index("t", "N_t", N_t);
      current_statement__ = 44;
      context__.validate_dims("data initialization","t","double",
          context__.to_vec(N_t));
      t = std::vector<double>(N_t, std::numeric_limits<double>::quiet_NaN());
      
      current_statement__ = 44;
      assign(t, nil_index_list(), context__.vals_r("t"),
        "assigning variable t");
      current_statement__ = 45;
      validate_non_negative_index("s", "N", N);
      current_statement__ = 46;
      context__.validate_dims("data initialization","s","double",
          context__.to_vec(N));
      s = std::vector<double>(N, std::numeric_limits<double>::quiet_NaN());
      
      current_statement__ = 46;
      assign(s, nil_index_list(), context__.vals_r("s"),
        "assigning variable s");
      current_statement__ = 47;
      validate_non_negative_index("n", "N", N);
      current_statement__ = 48;
      context__.validate_dims("data initialization","n","int",
          context__.to_vec(N));
      n = std::vector<int>(N, std::numeric_limits<int>::min());
      
      current_statement__ = 48;
      assign(n, nil_index_list(), context__.vals_i("n"),
        "assigning variable n");
      current_statement__ = 49;
      validate_non_negative_index("k", "N", N);
      current_statement__ = 50;
      context__.validate_dims("data initialization","k","int",
          context__.to_vec(N));
      k = std::vector<int>(N, std::numeric_limits<int>::min());
      
      current_statement__ = 50;
      assign(k, nil_index_list(), context__.vals_i("k"),
        "assigning variable k");
      current_statement__ = 51;
      context__.validate_dims("data initialization","J_sens","int",
          context__.to_vec());
      J_sens = std::numeric_limits<int>::min();
      
      current_statement__ = 51;
      J_sens = context__.vals_i("J_sens")[(1 - 1)];
      current_statement__ = 51;
      current_statement__ = 51;
      check_greater_or_equal(function__, "J_sens", J_sens, 0);
      current_statement__ = 52;
      validate_non_negative_index("y_sens", "J_sens", J_sens);
      current_statement__ = 53;
      context__.validate_dims("data initialization","y_sens","int",
          context__.to_vec(J_sens));
      y_sens = std::vector<int>(J_sens, std::numeric_limits<int>::min());
      
      current_statement__ = 53;
      assign(y_sens, nil_index_list(), context__.vals_i("y_sens"),
        "assigning variable y_sens");
      current_statement__ = 53;
      for (int sym1__ = 1; sym1__ <= J_sens; ++sym1__) {
        current_statement__ = 53;
        current_statement__ = 53;
        check_greater_or_equal(function__, "y_sens[sym1__]",
                               y_sens[(sym1__ - 1)], 0);}
      current_statement__ = 54;
      validate_non_negative_index("n_sens", "J_sens", J_sens);
      current_statement__ = 55;
      context__.validate_dims("data initialization","n_sens","int",
          context__.to_vec(J_sens));
      n_sens = std::vector<int>(J_sens, std::numeric_limits<int>::min());
      
      current_statement__ = 55;
      assign(n_sens, nil_index_list(), context__.vals_i("n_sens"),
        "assigning variable n_sens");
      current_statement__ = 55;
      for (int sym1__ = 1; sym1__ <= J_sens; ++sym1__) {
        current_statement__ = 55;
        current_statement__ = 55;
        check_greater_or_equal(function__, "n_sens[sym1__]",
                               n_sens[(sym1__ - 1)], 0);}
      current_statement__ = 56;
      context__.validate_dims("data initialization","p_sens","double",
          context__.to_vec(2));
      p_sens = std::vector<double>(2, std::numeric_limits<double>::quiet_NaN());
      
      current_statement__ = 56;
      assign(p_sens, nil_index_list(), context__.vals_r("p_sens"),
        "assigning variable p_sens");
      current_statement__ = 57;
      context__.validate_dims("data initialization","p_intercept","double",
          context__.to_vec(2));
      p_intercept = std::vector<double>(2, std::numeric_limits<double>::quiet_NaN());
      
      current_statement__ = 57;
      assign(p_intercept, nil_index_list(), context__.vals_r("p_intercept"),
        "assigning variable p_intercept");
      current_statement__ = 58;
      context__.validate_dims("data initialization","spec","double",
          context__.to_vec());
      spec = std::numeric_limits<double>::quiet_NaN();
      
      current_statement__ = 58;
      spec = context__.vals_r("spec")[(1 - 1)];
      current_statement__ = 59;
      context__.validate_dims("data initialization","N_phi","int",
          context__.to_vec());
      N_phi = std::numeric_limits<int>::min();
      
      current_statement__ = 59;
      N_phi = context__.vals_i("N_phi")[(1 - 1)];
      current_statement__ = 60;
      validate_non_negative_index("use_phi", "N_pop", N_pop);
      current_statement__ = 61;
      context__.validate_dims("data initialization","use_phi","int",
          context__.to_vec(N_pop));
      use_phi = std::vector<int>(N_pop, std::numeric_limits<int>::min());
      
      current_statement__ = 61;
      assign(use_phi, nil_index_list(), context__.vals_i("use_phi"),
        "assigning variable use_phi");
      current_statement__ = 62;
      context__.validate_dims("data initialization","p_phi","double",
          context__.to_vec());
      p_phi = std::numeric_limits<double>::quiet_NaN();
      
      current_statement__ = 62;
      p_phi = context__.vals_r("p_phi")[(1 - 1)];
      current_statement__ = 63;
      context__.validate_dims("data initialization","inference","int",
          context__.to_vec());
      inference = std::numeric_limits<int>::min();
      
      current_statement__ = 63;
      inference = context__.vals_i("inference")[(1 - 1)];
      current_statement__ = 64;
      delta = std::numeric_limits<double>::quiet_NaN();
      
      current_statement__ = 64;
      delta = 1e-9;
      current_statement__ = 65;
      validate_non_negative_index("logit_prev_f", "N_pop", N_pop);
      current_statement__ = 66;
      validate_non_negative_index("logit_prev_f", "N_t", N_t);
      current_statement__ = 67;
      validate_non_negative_index("phi", "N_phi", N_phi);
      current_statement__ = 68;
      validate_non_negative_index("kappa", "N_phi", N_phi);
      current_statement__ = 69;
      validate_non_negative_index("prev_f", "N_pop", N_pop);
      current_statement__ = 70;
      validate_non_negative_index("prev_f", "N_t", N_t);
      current_statement__ = 71;
      validate_non_negative_index("pool_pos", "N", N);
      current_statement__ = 72;
      validate_non_negative_index("prev_ratio", "N_pop", N_pop);
      current_statement__ = 73;
      prev_ratio_2dim__ = std::numeric_limits<int>::min();
      
      current_statement__ = 73;
      prev_ratio_2dim__ = (N_t - 1);
      current_statement__ = 73;
      validate_non_negative_index("prev_ratio", "N_t - 1", prev_ratio_2dim__);
    } catch (const std::exception& e) {
      stan::lang::rethrow_located(e, locations_array__[current_statement__]);
      // Next line prevents compiler griping about no return
      throw std::runtime_error("*** IF YOU SEE THIS, PLEASE REPORT A BUG ***"); 
    }
    num_params_r__ = 0U;
    
    try {
      num_params_r__ += N_pop * N_t;
      num_params_r__ += 1;
      num_params_r__ += N_phi;
    } catch (const std::exception& e) {
      stan::lang::rethrow_located(e, locations_array__[current_statement__]);
      // Next line prevents compiler griping about no return
      throw std::runtime_error("*** IF YOU SEE THIS, PLEASE REPORT A BUG ***"); 
    }
  }
  template <bool propto__, bool jacobian__, typename VecR, typename VecI, stan::require_vector_like_t<VecR>* = nullptr, stan::require_vector_like_vt<std::is_integral, VecI>* = nullptr>
  inline stan::scalar_type_t<VecR> log_prob_impl(VecR& params_r__,
                                                 VecI& params_i__,
                                                 std::ostream* pstream__ = nullptr) const {
    using T__ = stan::scalar_type_t<VecR>;
    using local_scalar_t__ = T__;
    T__ lp__(0.0);
    stan::math::accumulator<T__> lp_accum__;
    static const char* function__ = "model_mod_timepoint_overdisp_ind_spec_namespace::log_prob";
(void) function__;  // suppress unused var warning
    stan::io::reader<local_scalar_t__> in__(params_r__, params_i__);
    local_scalar_t__ DUMMY_VAR__(std::numeric_limits<double>::quiet_NaN());
    (void) DUMMY_VAR__;  // suppress unused var warning
    
    try {
      std::vector<Eigen::Matrix<local_scalar_t__, -1, 1>> logit_prev_f;
      logit_prev_f = std::vector<Eigen::Matrix<local_scalar_t__, -1, 1>>(N_pop, Eigen::Matrix<local_scalar_t__, -1, 1>(N_t));
      stan::math::fill(logit_prev_f, DUMMY_VAR__);
      
      current_statement__ = 1;
      for (int sym1__ = 1; sym1__ <= N_pop; ++sym1__) {
        current_statement__ = 1;
        assign(logit_prev_f, cons_list(index_uni(sym1__), nil_index_list()),
          in__.vector(N_t), "assigning variable logit_prev_f");}
      local_scalar_t__ sens;
      sens = DUMMY_VAR__;
      
      current_statement__ = 2;
      sens = in__.scalar();
      current_statement__ = 2;
      if (jacobian__) {
        current_statement__ = 2;
        sens = stan::math::lub_constrain(sens, 0, 1, lp__);
      } else {
        current_statement__ = 2;
        sens = stan::math::lub_constrain(sens, 0, 1);
      }
      Eigen::Matrix<local_scalar_t__, -1, 1> phi;
      phi = Eigen::Matrix<local_scalar_t__, -1, 1>(N_phi);
      stan::math::fill(phi, DUMMY_VAR__);
      
      current_statement__ = 3;
      phi = in__.vector(N_phi);
      current_statement__ = 3;
      for (int sym1__ = 1; sym1__ <= N_phi; ++sym1__) {
        current_statement__ = 3;
        if (jacobian__) {
          current_statement__ = 3;
          assign(phi, cons_list(index_uni(sym1__), nil_index_list()),
            stan::math::lb_constrain(phi[(sym1__ - 1)], 0, lp__),
            "assigning variable phi");
        } else {
          current_statement__ = 3;
          assign(phi, cons_list(index_uni(sym1__), nil_index_list()),
            stan::math::lb_constrain(phi[(sym1__ - 1)], 0),
            "assigning variable phi");
        }}
      Eigen::Matrix<local_scalar_t__, -1, 1> kappa;
      kappa = Eigen::Matrix<local_scalar_t__, -1, 1>(N_phi);
      stan::math::fill(kappa, DUMMY_VAR__);
      
      current_statement__ = 4;
      assign(kappa, nil_index_list(), add(phi, 2.0),
        "assigning variable kappa");
      std::vector<Eigen::Matrix<local_scalar_t__, -1, 1>> prev_f;
      prev_f = std::vector<Eigen::Matrix<local_scalar_t__, -1, 1>>(N_pop, Eigen::Matrix<local_scalar_t__, -1, 1>(N_t));
      stan::math::fill(prev_f, DUMMY_VAR__);
      
      std::vector<local_scalar_t__> pool_pos;
      pool_pos = std::vector<local_scalar_t__>(N, DUMMY_VAR__);
      
      current_statement__ = 9;
      for (int i = 1; i <= N_pop; ++i) {
        current_statement__ = 7;
        assign(prev_f, cons_list(index_uni(i), nil_index_list()),
          inv_logit(logit_prev_f[(i - 1)]), "assigning variable prev_f");}
      current_statement__ = 12;
      for (int l = 1; l <= N; ++l) {
        current_statement__ = 10;
        assign(pool_pos, cons_list(index_uni(l), nil_index_list()),
          (1.0 -
            pow(
              (1.0 -
                ((prev_f[(pop[(l - 1)] - 1)][(rank_t[(l - 1)] - 1)] * sens) +
                  ((1.0 - prev_f[(pop[(l - 1)] - 1)][(rank_t[(l - 1)] - 1)])
                    * (1.0 - spec)))), s[(l - 1)])),
          "assigning variable pool_pos");}
      current_statement__ = 4;
      for (int sym1__ = 1; sym1__ <= N_phi; ++sym1__) {
        current_statement__ = 4;
        current_statement__ = 4;
        check_greater_or_equal(function__, "kappa[sym1__]",
                               kappa[(sym1__ - 1)], 0);}
      {
        current_statement__ = 21;
        for (int i = 1; i <= N_pop; ++i) {
          current_statement__ = 19;
          lp_accum__.add(
            normal_lpdf<propto__>(logit_prev_f[(i - 1)],
              p_intercept[(1 - 1)], p_intercept[(2 - 1)]));}
        current_statement__ = 22;
        lp_accum__.add(exponential_lpdf<propto__>(phi, p_phi));
        current_statement__ = 23;
        lp_accum__.add(
          beta_lpdf<propto__>(sens, p_sens[(1 - 1)], p_sens[(2 - 1)]));
        current_statement__ = 35;
        if (logical_eq(inference, 1)) {
          current_statement__ = 30;
          for (int i = 1; i <= N; ++i) {
            current_statement__ = 28;
            if (logical_gt(use_phi[(pop[(i - 1)] - 1)], 0)) {
              current_statement__ = 26;
              lp_accum__.add(
                beta_binomial_lpmf<false>(k[(i - 1)], n[(i - 1)],
                  (kappa[(use_phi[(pop[(i - 1)] - 1)] - 1)] *
                    pool_pos[(i - 1)]),
                  (kappa[(use_phi[(pop[(i - 1)] - 1)] - 1)] *
                    (1 - pool_pos[(i - 1)]))));
            } else {
              current_statement__ = 24;
              lp_accum__.add(
                binomial_lpmf<false>(k[(i - 1)], n[(i - 1)],
                  pool_pos[(i - 1)]));
            }}
          current_statement__ = 33;
          for (int i = 1; i <= J_sens; ++i) {
            current_statement__ = 31;
            lp_accum__.add(
              binomial_lpmf<false>(y_sens[(i - 1)], n_sens[(i - 1)], sens));}
        } 
      }
    } catch (const std::exception& e) {
      stan::lang::rethrow_located(e, locations_array__[current_statement__]);
      // Next line prevents compiler griping about no return
      throw std::runtime_error("*** IF YOU SEE THIS, PLEASE REPORT A BUG ***"); 
    }
    lp_accum__.add(lp__);
    return lp_accum__.sum();
    } // log_prob_impl() 
    
  template <typename RNG, typename VecR, typename VecI, typename VecVar, stan::require_vector_like_vt<std::is_floating_point, VecR>* = nullptr, stan::require_vector_like_vt<std::is_integral, VecI>* = nullptr, stan::require_std_vector_vt<std::is_floating_point, VecVar>* = nullptr>
  inline void write_array_impl(RNG& base_rng__, VecR& params_r__,
                               VecI& params_i__, VecVar& vars__,
                               const bool emit_transformed_parameters__ = true,
                               const bool emit_generated_quantities__ = true,
                               std::ostream* pstream__ = nullptr) const {
    using local_scalar_t__ = double;
    vars__.resize(0);
    stan::io::reader<local_scalar_t__> in__(params_r__, params_i__);
    static const char* function__ = "model_mod_timepoint_overdisp_ind_spec_namespace::write_array";
(void) function__;  // suppress unused var warning
    (void) function__;  // suppress unused var warning
    double lp__ = 0.0;
    (void) lp__;  // dummy to suppress unused var warning
    stan::math::accumulator<double> lp_accum__;
    local_scalar_t__ DUMMY_VAR__(std::numeric_limits<double>::quiet_NaN());
    (void) DUMMY_VAR__;  // suppress unused var warning
    
    try {
      std::vector<Eigen::Matrix<double, -1, 1>> logit_prev_f;
      logit_prev_f = std::vector<Eigen::Matrix<double, -1, 1>>(N_pop, Eigen::Matrix<double, -1, 1>(N_t));
      stan::math::fill(logit_prev_f, std::numeric_limits<double>::quiet_NaN());
      
      current_statement__ = 1;
      for (int sym1__ = 1; sym1__ <= N_pop; ++sym1__) {
        current_statement__ = 1;
        assign(logit_prev_f, cons_list(index_uni(sym1__), nil_index_list()),
          in__.vector(N_t), "assigning variable logit_prev_f");}
      double sens;
      sens = std::numeric_limits<double>::quiet_NaN();
      
      current_statement__ = 2;
      sens = in__.scalar();
      current_statement__ = 2;
      sens = stan::math::lub_constrain(sens, 0, 1);
      Eigen::Matrix<double, -1, 1> phi;
      phi = Eigen::Matrix<double, -1, 1>(N_phi);
      stan::math::fill(phi, std::numeric_limits<double>::quiet_NaN());
      
      current_statement__ = 3;
      phi = in__.vector(N_phi);
      current_statement__ = 3;
      for (int sym1__ = 1; sym1__ <= N_phi; ++sym1__) {
        current_statement__ = 3;
        assign(phi, cons_list(index_uni(sym1__), nil_index_list()),
          stan::math::lb_constrain(phi[(sym1__ - 1)], 0),
          "assigning variable phi");}
      Eigen::Matrix<double, -1, 1> kappa;
      kappa = Eigen::Matrix<double, -1, 1>(N_phi);
      stan::math::fill(kappa, std::numeric_limits<double>::quiet_NaN());
      
      std::vector<Eigen::Matrix<double, -1, 1>> prev_f;
      prev_f = std::vector<Eigen::Matrix<double, -1, 1>>(N_pop, Eigen::Matrix<double, -1, 1>(N_t));
      stan::math::fill(prev_f, std::numeric_limits<double>::quiet_NaN());
      
      std::vector<double> pool_pos;
      pool_pos = std::vector<double>(N, std::numeric_limits<double>::quiet_NaN());
      
      for (int sym1__ = 1; sym1__ <= N_t; ++sym1__) {
        for (int sym2__ = 1; sym2__ <= N_pop; ++sym2__) {
          vars__.emplace_back(logit_prev_f[(sym2__ - 1)][(sym1__ - 1)]);}}
      vars__.emplace_back(sens);
      for (int sym1__ = 1; sym1__ <= N_phi; ++sym1__) {
        vars__.emplace_back(phi[(sym1__ - 1)]);}
      if (logical_negation((primitive_value(emit_transformed_parameters__) ||
            primitive_value(emit_generated_quantities__)))) {
        return ;
      } 
      current_statement__ = 4;
      assign(kappa, nil_index_list(), add(phi, 2.0),
        "assigning variable kappa");
      current_statement__ = 9;
      for (int i = 1; i <= N_pop; ++i) {
        current_statement__ = 7;
        assign(prev_f, cons_list(index_uni(i), nil_index_list()),
          inv_logit(logit_prev_f[(i - 1)]), "assigning variable prev_f");}
      current_statement__ = 12;
      for (int l = 1; l <= N; ++l) {
        current_statement__ = 10;
        assign(pool_pos, cons_list(index_uni(l), nil_index_list()),
          (1.0 -
            pow(
              (1.0 -
                ((prev_f[(pop[(l - 1)] - 1)][(rank_t[(l - 1)] - 1)] * sens) +
                  ((1.0 - prev_f[(pop[(l - 1)] - 1)][(rank_t[(l - 1)] - 1)])
                    * (1.0 - spec)))), s[(l - 1)])),
          "assigning variable pool_pos");}
      current_statement__ = 4;
      for (int sym1__ = 1; sym1__ <= N_phi; ++sym1__) {
        current_statement__ = 4;
        current_statement__ = 4;
        check_greater_or_equal(function__, "kappa[sym1__]",
                               kappa[(sym1__ - 1)], 0);}
      if (emit_transformed_parameters__) {
        for (int sym1__ = 1; sym1__ <= N_phi; ++sym1__) {
          vars__.emplace_back(kappa[(sym1__ - 1)]);}
        for (int sym1__ = 1; sym1__ <= N_t; ++sym1__) {
          for (int sym2__ = 1; sym2__ <= N_pop; ++sym2__) {
            vars__.emplace_back(prev_f[(sym2__ - 1)][(sym1__ - 1)]);}}
        for (int sym1__ = 1; sym1__ <= N; ++sym1__) {
          vars__.emplace_back(pool_pos[(sym1__ - 1)]);}
      } 
      if (logical_negation(emit_generated_quantities__)) {
        return ;
      } 
      std::vector<std::vector<double>> prev_ratio;
      prev_ratio = std::vector<std::vector<double>>(N_pop, std::vector<double>(prev_ratio_2dim__, std::numeric_limits<double>::quiet_NaN()));
      
      current_statement__ = 18;
      for (int i = 1; i <= N_pop; ++i) {
        current_statement__ = 16;
        for (int j = 1; j <= (N_t - 1); ++j) {
          current_statement__ = 14;
          assign(prev_ratio,
            cons_list(index_uni(i),
              cons_list(index_uni(j), nil_index_list())),
            (prev_f[(i - 1)][((j + 1) - 1)] / prev_f[(i - 1)][(j - 1)]),
            "assigning variable prev_ratio");}}
      for (int sym1__ = 1; sym1__ <= prev_ratio_2dim__; ++sym1__) {
        for (int sym2__ = 1; sym2__ <= N_pop; ++sym2__) {
          vars__.emplace_back(prev_ratio[(sym2__ - 1)][(sym1__ - 1)]);}}
    } catch (const std::exception& e) {
      stan::lang::rethrow_located(e, locations_array__[current_statement__]);
      // Next line prevents compiler griping about no return
      throw std::runtime_error("*** IF YOU SEE THIS, PLEASE REPORT A BUG ***"); 
    }
    } // write_array_impl() 
    
  template <typename VecVar, typename VecI, stan::require_std_vector_t<VecVar>* = nullptr, stan::require_vector_like_vt<std::is_integral, VecI>* = nullptr>
  inline void transform_inits_impl(const stan::io::var_context& context__,
                                   VecI& params_i__, VecVar& vars__,
                                   std::ostream* pstream__ = nullptr) const {
    using local_scalar_t__ = double;
    vars__.clear();
    vars__.reserve(num_params_r__);
    
    try {
      int pos__;
      pos__ = std::numeric_limits<int>::min();
      
      pos__ = 1;
      std::vector<Eigen::Matrix<double, -1, 1>> logit_prev_f;
      logit_prev_f = std::vector<Eigen::Matrix<double, -1, 1>>(N_pop, Eigen::Matrix<double, -1, 1>(N_t));
      stan::math::fill(logit_prev_f, std::numeric_limits<double>::quiet_NaN());
      
      {
        std::vector<local_scalar_t__> logit_prev_f_flat__;
        current_statement__ = 1;
        assign(logit_prev_f_flat__, nil_index_list(),
          context__.vals_r("logit_prev_f"),
          "assigning variable logit_prev_f_flat__");
        current_statement__ = 1;
        pos__ = 1;
        current_statement__ = 1;
        for (int sym1__ = 1; sym1__ <= N_t; ++sym1__) {
          current_statement__ = 1;
          for (int sym2__ = 1; sym2__ <= N_pop; ++sym2__) {
            current_statement__ = 1;
            assign(logit_prev_f,
              cons_list(index_uni(sym2__),
                cons_list(index_uni(sym1__), nil_index_list())),
              logit_prev_f_flat__[(pos__ - 1)],
              "assigning variable logit_prev_f");
            current_statement__ = 1;
            pos__ = (pos__ + 1);}}
      }
      double sens;
      sens = std::numeric_limits<double>::quiet_NaN();
      
      current_statement__ = 2;
      sens = context__.vals_r("sens")[(1 - 1)];
      double sens_free__;
      sens_free__ = std::numeric_limits<double>::quiet_NaN();
      
      current_statement__ = 2;
      sens_free__ = stan::math::lub_free(sens, 0, 1);
      Eigen::Matrix<double, -1, 1> phi;
      phi = Eigen::Matrix<double, -1, 1>(N_phi);
      stan::math::fill(phi, std::numeric_limits<double>::quiet_NaN());
      
      {
        std::vector<local_scalar_t__> phi_flat__;
        current_statement__ = 3;
        assign(phi_flat__, nil_index_list(), context__.vals_r("phi"),
          "assigning variable phi_flat__");
        current_statement__ = 3;
        pos__ = 1;
        current_statement__ = 3;
        for (int sym1__ = 1; sym1__ <= N_phi; ++sym1__) {
          current_statement__ = 3;
          assign(phi, cons_list(index_uni(sym1__), nil_index_list()),
            phi_flat__[(pos__ - 1)], "assigning variable phi");
          current_statement__ = 3;
          pos__ = (pos__ + 1);}
      }
      Eigen::Matrix<double, -1, 1> phi_free__;
      phi_free__ = Eigen::Matrix<double, -1, 1>(N_phi);
      stan::math::fill(phi_free__, std::numeric_limits<double>::quiet_NaN());
      
      current_statement__ = 3;
      for (int sym1__ = 1; sym1__ <= N_phi; ++sym1__) {
        current_statement__ = 3;
        assign(phi_free__, cons_list(index_uni(sym1__), nil_index_list()),
          stan::math::lb_free(phi[(sym1__ - 1)], 0),
          "assigning variable phi_free__");}
      for (int sym1__ = 1; sym1__ <= N_pop; ++sym1__) {
        for (int sym2__ = 1; sym2__ <= N_t; ++sym2__) {
          vars__.emplace_back(logit_prev_f[(sym1__ - 1)][(sym2__ - 1)]);}}
      vars__.emplace_back(sens_free__);
      for (int sym1__ = 1; sym1__ <= N_phi; ++sym1__) {
        vars__.emplace_back(phi_free__[(sym1__ - 1)]);}
    } catch (const std::exception& e) {
      stan::lang::rethrow_located(e, locations_array__[current_statement__]);
      // Next line prevents compiler griping about no return
      throw std::runtime_error("*** IF YOU SEE THIS, PLEASE REPORT A BUG ***"); 
    }
    } // transform_inits_impl() 
    
  inline void get_param_names(std::vector<std::string>& names__) const {
    
    names__.clear();
    names__.emplace_back("logit_prev_f");
    names__.emplace_back("sens");
    names__.emplace_back("phi");
    names__.emplace_back("kappa");
    names__.emplace_back("prev_f");
    names__.emplace_back("pool_pos");
    names__.emplace_back("prev_ratio");
    } // get_param_names() 
    
  inline void get_dims(std::vector<std::vector<size_t>>& dimss__) const {
    dimss__.clear();
    dimss__.emplace_back(std::vector<size_t>{static_cast<size_t>(N_pop),
                                             static_cast<size_t>(N_t)});
    
    dimss__.emplace_back(std::vector<size_t>{});
    
    dimss__.emplace_back(std::vector<size_t>{static_cast<size_t>(N_phi)});
    
    dimss__.emplace_back(std::vector<size_t>{static_cast<size_t>(N_phi)});
    
    dimss__.emplace_back(std::vector<size_t>{static_cast<size_t>(N_pop),
                                             static_cast<size_t>(N_t)});
    
    dimss__.emplace_back(std::vector<size_t>{static_cast<size_t>(N)});
    
    dimss__.emplace_back(std::vector<size_t>{static_cast<size_t>(N_pop),
                                             static_cast<size_t>(prev_ratio_2dim__)
                                             });
    
    } // get_dims() 
    
  inline void constrained_param_names(
                                      std::vector<std::string>& param_names__,
                                      bool emit_transformed_parameters__ = true,
                                      bool emit_generated_quantities__ = true) const
    final {
    
    for (int sym1__ = 1; sym1__ <= N_t; ++sym1__) {
      {
        for (int sym2__ = 1; sym2__ <= N_pop; ++sym2__) {
          {
            param_names__.emplace_back(std::string() + "logit_prev_f" + '.' + std::to_string(sym2__) + '.' + std::to_string(sym1__));
          }}
      }}
    param_names__.emplace_back(std::string() + "sens");
    for (int sym1__ = 1; sym1__ <= N_phi; ++sym1__) {
      {
        param_names__.emplace_back(std::string() + "phi" + '.' + std::to_string(sym1__));
      }}
    if (emit_transformed_parameters__) {
      for (int sym1__ = 1; sym1__ <= N_phi; ++sym1__) {
        {
          param_names__.emplace_back(std::string() + "kappa" + '.' + std::to_string(sym1__));
        }}
      for (int sym1__ = 1; sym1__ <= N_t; ++sym1__) {
        {
          for (int sym2__ = 1; sym2__ <= N_pop; ++sym2__) {
            {
              param_names__.emplace_back(std::string() + "prev_f" + '.' + std::to_string(sym2__) + '.' + std::to_string(sym1__));
            }}
        }}
      for (int sym1__ = 1; sym1__ <= N; ++sym1__) {
        {
          param_names__.emplace_back(std::string() + "pool_pos" + '.' + std::to_string(sym1__));
        }}
    }
    
    if (emit_generated_quantities__) {
      for (int sym1__ = 1; sym1__ <= prev_ratio_2dim__; ++sym1__) {
        {
          for (int sym2__ = 1; sym2__ <= N_pop; ++sym2__) {
            {
              param_names__.emplace_back(std::string() + "prev_ratio" + '.' + std::to_string(sym2__) + '.' + std::to_string(sym1__));
            }}
        }}
    }
    
    } // constrained_param_names() 
    
  inline void unconstrained_param_names(
                                        std::vector<std::string>& param_names__,
                                        bool emit_transformed_parameters__ = true,
                                        bool emit_generated_quantities__ = true) const
    final {
    
    for (int sym1__ = 1; sym1__ <= N_t; ++sym1__) {
      {
        for (int sym2__ = 1; sym2__ <= N_pop; ++sym2__) {
          {
            param_names__.emplace_back(std::string() + "logit_prev_f" + '.' + std::to_string(sym2__) + '.' + std::to_string(sym1__));
          }}
      }}
    param_names__.emplace_back(std::string() + "sens");
    for (int sym1__ = 1; sym1__ <= N_phi; ++sym1__) {
      {
        param_names__.emplace_back(std::string() + "phi" + '.' + std::to_string(sym1__));
      }}
    if (emit_transformed_parameters__) {
      for (int sym1__ = 1; sym1__ <= N_phi; ++sym1__) {
        {
          param_names__.emplace_back(std::string() + "kappa" + '.' + std::to_string(sym1__));
        }}
      for (int sym1__ = 1; sym1__ <= N_t; ++sym1__) {
        {
          for (int sym2__ = 1; sym2__ <= N_pop; ++sym2__) {
            {
              param_names__.emplace_back(std::string() + "prev_f" + '.' + std::to_string(sym2__) + '.' + std::to_string(sym1__));
            }}
        }}
      for (int sym1__ = 1; sym1__ <= N; ++sym1__) {
        {
          param_names__.emplace_back(std::string() + "pool_pos" + '.' + std::to_string(sym1__));
        }}
    }
    
    if (emit_generated_quantities__) {
      for (int sym1__ = 1; sym1__ <= prev_ratio_2dim__; ++sym1__) {
        {
          for (int sym2__ = 1; sym2__ <= N_pop; ++sym2__) {
            {
              param_names__.emplace_back(std::string() + "prev_ratio" + '.' + std::to_string(sym2__) + '.' + std::to_string(sym1__));
            }}
        }}
    }
    
    } // unconstrained_param_names() 
    
  inline std::string get_constrained_sizedtypes() const {
    stringstream s__;
    s__ << "[{\"name\":\"logit_prev_f\",\"type\":{\"name\":\"array\",\"length\":" << N_pop << ",\"element_type\":{\"name\":\"vector\",\"length\":" << N_t << "}},\"block\":\"parameters\"},{\"name\":\"sens\",\"type\":{\"name\":\"real\"},\"block\":\"parameters\"},{\"name\":\"phi\",\"type\":{\"name\":\"vector\",\"length\":" << N_phi << "},\"block\":\"parameters\"},{\"name\":\"kappa\",\"type\":{\"name\":\"vector\",\"length\":" << N_phi << "},\"block\":\"transformed_parameters\"},{\"name\":\"prev_f\",\"type\":{\"name\":\"array\",\"length\":" << N_pop << ",\"element_type\":{\"name\":\"vector\",\"length\":" << N_t << "}},\"block\":\"transformed_parameters\"},{\"name\":\"pool_pos\",\"type\":{\"name\":\"array\",\"length\":" << N << ",\"element_type\":{\"name\":\"real\"}},\"block\":\"transformed_parameters\"},{\"name\":\"prev_ratio\",\"type\":{\"name\":\"array\",\"length\":" << N_pop << ",\"element_type\":{\"name\":\"array\",\"length\":" << prev_ratio_2dim__ << ",\"element_type\":{\"name\":\"real\"}}},\"block\":\"generated_quantities\"}]";
    return s__.str();
    } // get_constrained_sizedtypes() 
    
  inline std::string get_unconstrained_sizedtypes() const {
    stringstream s__;
    s__ << "[{\"name\":\"logit_prev_f\",\"type\":{\"name\":\"array\",\"length\":" << N_pop << ",\"element_type\":{\"name\":\"vector\",\"length\":" << N_t << "}},\"block\":\"parameters\"},{\"name\":\"sens\",\"type\":{\"name\":\"real\"},\"block\":\"parameters\"},{\"name\":\"phi\",\"type\":{\"name\":\"vector\",\"length\":" << N_phi << "},\"block\":\"parameters\"},{\"name\":\"kappa\",\"type\":{\"name\":\"vector\",\"length\":" << N_phi << "},\"block\":\"transformed_parameters\"},{\"name\":\"prev_f\",\"type\":{\"name\":\"array\",\"length\":" << N_pop << ",\"element_type\":{\"name\":\"vector\",\"length\":" << N_t << "}},\"block\":\"transformed_parameters\"},{\"name\":\"pool_pos\",\"type\":{\"name\":\"array\",\"length\":" << N << ",\"element_type\":{\"name\":\"real\"}},\"block\":\"transformed_parameters\"},{\"name\":\"prev_ratio\",\"type\":{\"name\":\"array\",\"length\":" << N_pop << ",\"element_type\":{\"name\":\"array\",\"length\":" << prev_ratio_2dim__ << ",\"element_type\":{\"name\":\"real\"}}},\"block\":\"generated_quantities\"}]";
    return s__.str();
    } // get_unconstrained_sizedtypes() 
    
  
    // Begin method overload boilerplate
    template <typename RNG>
    inline void write_array(RNG& base_rng,
                            Eigen::Matrix<double,Eigen::Dynamic,1>& params_r,
                            Eigen::Matrix<double,Eigen::Dynamic,1>& vars,
                            const bool emit_transformed_parameters = true,
                            const bool emit_generated_quantities = true,
                            std::ostream* pstream = nullptr) const {
      std::vector<double> vars_vec(vars.size());
      std::vector<int> params_i;
      write_array_impl(base_rng, params_r, params_i, vars_vec,
          emit_transformed_parameters, emit_generated_quantities, pstream);
      vars.resize(vars_vec.size());
      for (int i = 0; i < vars.size(); ++i) {
        vars.coeffRef(i) = vars_vec[i];
      }
    }
    template <typename RNG>
    inline void write_array(RNG& base_rng, std::vector<double>& params_r,
                            std::vector<int>& params_i,
                            std::vector<double>& vars,
                            bool emit_transformed_parameters = true,
                            bool emit_generated_quantities = true,
                            std::ostream* pstream = nullptr) const {
      write_array_impl(base_rng, params_r, params_i, vars, emit_transformed_parameters, emit_generated_quantities, pstream);
    }
    template <bool propto__, bool jacobian__, typename T_>
    inline T_ log_prob(Eigen::Matrix<T_,Eigen::Dynamic,1>& params_r,
                       std::ostream* pstream = nullptr) const {
      Eigen::Matrix<int, -1, 1> params_i;
      return log_prob_impl<propto__, jacobian__>(params_r, params_i, pstream);
    }
    template <bool propto__, bool jacobian__, typename T__>
    inline T__ log_prob(std::vector<T__>& params_r,
                        std::vector<int>& params_i,
                        std::ostream* pstream = nullptr) const {
      return log_prob_impl<propto__, jacobian__>(params_r, params_i, pstream);
    }
  
    inline void transform_inits(const stan::io::var_context& context,
                         Eigen::Matrix<double, Eigen::Dynamic, 1>& params_r,
                         std::ostream* pstream = nullptr) const final {
      std::vector<double> params_r_vec(params_r.size());
      std::vector<int> params_i;
      transform_inits_impl(context, params_i, params_r_vec, pstream);
      params_r.resize(params_r_vec.size());
      for (int i = 0; i < params_r.size(); ++i) {
        params_r.coeffRef(i) = params_r_vec[i];
      }
    }
    inline void transform_inits(const stan::io::var_context& context,
                                std::vector<int>& params_i,
                                std::vector<double>& vars,
                                std::ostream* pstream = nullptr) const final {
      transform_inits_impl(context, params_i, vars, pstream);
    }        
};
}
using stan_model = model_mod_timepoint_overdisp_ind_spec_namespace::model_mod_timepoint_overdisp_ind_spec;
#ifndef USING_R
// Boilerplate
stan::model::model_base& new_model(
        stan::io::var_context& data_context,
        unsigned int seed,
        std::ostream* msg_stream) {
  stan_model* m = new stan_model(data_context, seed, msg_stream);
  return *m;
}
stan::math::profile_map& get_stan_profile_data() {
  return model_mod_timepoint_overdisp_ind_spec_namespace::profiles__;
}
#endif
#endif
